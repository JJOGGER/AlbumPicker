 ...
 */
class ImageRequest private constructor(context: Context,target: ImageView?){
...
internal var clipType = Base64.Decoder.NO_CLIP
...
fun scaleType(scaleType: ImageView.ScaleType): Request {
    this.clipType = Base64.Decoder.mapScaleType(scaleType)
    return this
}
...
fun into(target: ImageView?) {
    ...
    if (!noClip && clipType == Base64.Decoder.NO_CLIP) {
        clipType = Base64.Decoder.mapScaleType(target.scaleType)
    }
...
class  Builder{
    ...
    fun build():ImageRequest{
        return ImageRequest(context=context,target = target)
    }
